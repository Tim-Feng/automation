...
function handle_video_meta_update($post, $request) {
    ...
    // 處理 video_description
    if (isset($params['meta']['video_description'])) {
        update_post_meta($post->ID, 'video_description', $params['meta']['video_description']);
    }
...
function add_video_meta_to_rest_api($response, $post, $request) {
    ...
    $response->data['meta']['video_description'] = $video_description;
...
function register_video_meta_fields() {
    ...
    // 註冊 video_description 欄位
    register_meta('post', 'video_description', array(
        'object_subtype' => 'video',
        'show_in_rest' => true,
        'single' => true,
        'type' => 'string',
        'description' => '影片描述（由 Gemini 生成）',
        'auth_callback' => function() {
            return current_user_can('edit_posts');
        }
    ));
}
add_action('init', 'register_video_meta_fields');
